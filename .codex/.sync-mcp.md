...
根据最新官方文档审计（2025-05-06），以下工具配置已验证：
- ✅ Claude Code: 完全兼容
- ✅ Cursor: 完全兼容  
- ✅ GitHub Copilot CLI: 完全兼容
- ✅ Visual Studio Code: 完全兼容
- ✅ Gemini CLI: 完全兼容
- ⚠️ OpenAI Codex: 配置已更新为跨平台格式（见下文 5.1）

**关键变更**：Codex 配置不再使用 Windows 特定的 `cmd /c` 格式，改为跨平台的 `npx` 直接执行方式。
...
**说明**：
- 第一个配置使用本地 npx 执行，跨平台兼容
- 第二个配置使用 HTTP 远程服务，无需本地 Node.js 安装
- 推荐在 Linux/macOS/Windows 上统一使用第一个配置
...
**说明**：
- OpenCode 支持两种服务器类型：`stdio`（本地命令执行）和 `sse`（远程服务）
- 使用 `type: "stdio"` 时需要指定 `command` 和 `args`
- 使用 `type: "sse"` 时需要指定 `url`
